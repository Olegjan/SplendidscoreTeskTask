...
| Назва, підказка старту<br>і нагадування про жести | Монетка праворуч, перешкоди<br>попереду.<br>У HUD життя (♥♥♡) і рахунок | Біг зупинено, персонаж<br>у позі очікування; рахунок<br>заїзду проти рекорду сесії |

## Відповідність завданню

| Вимога | Стан |
|---|---|
| Bitmoji з анімаціями | біг + strafe-нахили при зміні смуги, поза очікування поза грою |
| Перешкоди, які можна оббігати й перестрибувати | три типи: стіна, низька, підвісна (+ підкат) |
| Зіткнення віднімає життя | 1.5 с недоторканності, і рівно стільки ж триває мигання |
...
| Свайп вгору | Стрибок — перестрибнути низьку перешкоду (дає ривок швидкості) |
...
```
Camera Object            перспективна камера (0, 15, 50), нахил -6°
...
(250 см/с, а під ривком за вдалий стрибок — до ~360) обʼєкт долає 8–12 см за кадр при
30 FPS, тож тонкий колайдер можна проскочити між кадрами. Перевірка «та сама смуга +
відстань по Z» детермінована, не залежить від FPS і легко дебажиться. Тип перешкоди
задає й спосіб ухилитись — це одне місце в коді (`TrackItem.isNotAvoided`), а не
розмазана по проєкту логіка.
...
**Тип перешкоди задає лічильник, а не порядок у пулі.** Спершу спавнер брав
перший вільний обʼєкт пулу, укладеного по колу `стіна → низька → підвісна`. На
папері це давало рівне чергування, а в грі — ні: що швидший світ, то раніше
обʼєкт доїжджає до `despawnZ`, і на момент спавну вільним щоразу встигав стати
той самий низькоіндексний. Замір показав, що після ~10 с підвісні перешкоди
зникали з доріжки зовсім. Тепер тип обирає лічильник, а пул лише постачає обʼєкт
потрібного типу; якщо всі обʼєкти цього типу ще в дорозі, береться наступний —
діра в доріжці помітніша за збій черги.
...
**Вдалий стрибок дає ривок швидкості.** Щоб його нарахувати, знадобилося те, чого в
коді не було: розпізнавання **вдалого** ухилення. Раніше `TrackItem` відповідав лише
на питання «чи це влучання», і успішний стрибок був для гри невидимим — просто
відсутністю події. Тепер `resolveEncounter()` повертає `Hit`, `Cleared` або `None`,
і обʼєкт закривається в мить, коли гравець входить у його зону: приземлення посеред
зони не має заднім числом перетворювати вдалий стрибок на зіткнення.

Множник (×1.45) саме множиться на базову швидкість, а не додається: так ривок
відчувається однаково і на старті, і коли світ уже розігнався. Стеля `maxSpeed` на
нього навмисно не поширюється — це коротка нагорода, а не новий рівень складності.
Далі множник лінійно спадає до 1 за 0.9 с.

Нараховується лише за стрибок. Підкат теж повертає `Cleared`, але ривка не дає —
так було в завданні; якщо треба, це один рядок у `TrackSpawner`.

**Мигання після удару — не на останньому зіткненні.** Після зіткнення персонаж блимає
1.5 с, перемикаючи видимість кожні 0.1 с. Але на третьому зіткненні мигання не
запускається: одразу за ним іде поза очікування, і блимання поверх неї читалося б як
збій рендеру, а не як фідбек. Розвилка природно лягла на вже наявну гілку
`lives <= 0 ? endGame() : startHitFlash()`, тож окремої перевірки «чи це третій удар»
не знадобилось.

Фаза мигання рахується від накопиченого часу, а не перемикається щокадру — інакше
частота залежала б від FPS. `stopHitFlash()` завжди повертає видимість, і його
викликають також `enterIdle()` та `reset()`: інакше персонаж міг би лишитись
невидимим, якщо гра завершиться посеред спалаху.
...
Ту саму стійку показує і стартовий екран, а не лише Game Over: до першого тапу
доріжка не рухається, тож біг на місці читався б як гра, що вже почалась.

**Перешкоди їдуть у бік `+Z`.** У Lens Studio `-Z` — це «вперед», а камера стоїть на
`z=50` позаду гравця. Обʼєкти народжуються на `z=-340` і рухаються до камери.
...
Після зіткнення діє 1.5 с недоторканності — широка перешкода перекриває гравця кілька
кадрів поспіль, і без цього три життя зникали б за мить. Мигання триває рівно стільки ж:
воно і є візуальним сигналом недоторканності, тож розбіжність між цими двома числами
обманювала б гравця. У `GameConfig` вони стоять поруч із поміткою тримати їх рівними.
...
| Зміна смуги в стрибку | дозволена, керування в повітрі не блокується | смуга змінена на висоті 12.7 см |
| Стрибок | пік = `CONFIG.jumpHeight` = 15 см | 15.0 см |
| Підкат | `0.15 × 0.45` | `scaleY = 0.068` |
| Стіна | не оминається ні стрибком, ні підкатом | `Hit` на висоті 14.5 см; `Hit` у підкаті |
| Свайп угору двічі за 120 мс | один звук (стрибок ще триває) | один |
| Свайп угору посеред підкату | ігнорується, а не стає в чергу | стрибок не почався ні під час підкату, ні після нього |
...
| Недоторканність після удару | наступний удар у межах 1.5 с не знімає життя | удари через 1.25 і 1.22 с відкинуто, життя без змін |
| Мигання після удару | перемикання кожні 0.1 с | крок 0.10 с, вимір на тривалості 2 с |
| Мигання на третьому ударі | не запускається | 2 спалахи на 3 зіткнення |
| Ривок за стрибок | старт ×1.45, спад до 1 | 1.45 → 1.43 → 1.42 → … |
| Ривок за підкат | не нараховується | `SlideUnder → Cleared`, ривка немає |
| Стеля `maxSpeed` і ривок | ривок кладеться поверх стелі | база 120.0 (стеля), ефективна 174.0 |
| Рестарт після Game Over | стан заїзду обнуляється, рекорд лишається | `score 0`, `♥♥♥`, смуга 1, `scaleY 0.150`, пул порожній, 110 см/с, `best 64` |
| Ріст складності | швидкість і інтервал ростуть лінійно й одночасно, далі стеля | t=13 с: 143.8 см/с і 1.194 с — точно за формулами; на прискореному розгоні сідають на 250.0 і 0.550 і завмирають |
| Бали за час | 2 за секунду | t=1 → 2, t=2 → 4, t=3 → 6 |
| Ціна монетки | 10 | секунда з підбором дає +12 замість +2 |
| Монетка в польоті | підбирається, висота не заважає | підбори на висоті 2.5 і 9.3 см |
| Despawn | за `despawnZ` обʼєкт вимикається і повертається в пул | 2–3 повернення на обʼєкт за 15 с, `active` і видимість зняті |
| Пул на максимальній швидкості | не вичерпується, такти не пропускаються | 24 с на стелі 250 см/с і 0.55 с: пік 3/10 перешкод і 4/14 монеток |
| Монетка не в смузі перешкоди | смуга не збігається з останньою перешкодою | 16 спавнів поспіль без збігу; до найближчої перешкоди тієї ж смуги мінімум 178 см при зоні підбору ±9 |
| Чергування типів перешкод | строге коло, після рестарту — з початку | два заїзди по 9 спавнів: стіна → низька → підвісна без збоїв, обидва починаються зі стіни |
| Оновлення UI | екрани перемикаються, HUD міняється наживо | Menu → HUD → GameOver; рахунок 8 → 41, життя `♥♥♥` → `♥♡♡`; `SCORE 44` дорівнює останньому HUD |

Чотири помилки знайшлися лише запуском — не читанням коду:
