...
Assets/Scripts/
  ...
  AudioManager.ts      — звуки монетки, зіткнення, стрибка та підкату
...
├── Audio                [AudioManager] → SFX_Coin, SFX_Hit, SFX_Jump, SFX_Slide
└── UI                   [UIManager]
...
заново. Усі звукові входи помічені `@allowUndefined`: гра має запускатися й мовчки.

**`jump()` і `slide()` повертають `boolean`.** Обидві дії ігноруються, якщо інша вже
триває, — і звук тоді грати не можна, інакше свайп під час стрибка «клацав» би без
жодного руху на екрані. Тому `PlayerController` повідомляє, чи дія справді почалася,
а `GameManager` вирішує, чи озвучувати її. Перевірено: два свайпи вгору з інтервалом
120 мс дають один звук, а не два.
...
`bubble_low` (зіткнення), `tap_jump` (стрибок), `bubble_mid` (підкат). Три «bubble»
дають узгоджену трійцю за висотою тону: високий — нагорода, середній — ухилення,
низький — помилка. Решта звуків пакета зі сцени видалені.
